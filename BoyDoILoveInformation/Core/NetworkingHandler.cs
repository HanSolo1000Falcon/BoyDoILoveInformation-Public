...
public class NetworkingHandler : MonoBehaviour
{
    ...
    private readonly Dictionary<VRRig, float> lastTimeOpened = new();
    ...
    private void Start()
    {
        ...
        BDILIUtils.OnPlayerCosmeticsLoaded += rig =>
                                              {
                                                  playerMenus[rig] = Instantiate(emptyMenuPrefab,
        ...
        PhotonNetwork.NetworkingClient.EventReceived += eventData =>
                                                        {
                                                            ...
                                                            if (sender == null || !playerMenus.ContainsKey(sender) ||
                                                                !eventData.Parameters.TryGetValue(ParameterCode.Data,
                                                                        out object data)     ||
                                                                data is not bool menuEnabled ||
                                                                lastTimeOpened[sender] + 0.05f > Time.time)
                                                                return;
